...
- **Stalker / Ministra Portal** accepts an HTTP/HTTPS portal address plus an authorized account or device MAC address.

M3U URLs may contain the common `username` and `password` query parameters supplied by a provider. Noki's IPTV Player removes those parameters before saving profile metadata, protects the credentials with Windows DPAPI, and reconstructs the request only when editing, testing, or loading that profile. Copied links containing `\&` or `&amp;` separators are normalized automatically. URL user-info and token/key parameters remain rejected. Xtream credentials and Stalker MAC addresses belong in their dedicated fields.
...
- A Stalker/Ministra quick test verifies base-server reachability. **Connect** performs an authorized MAC-address portal handshake and loads compatible live-channel data; see [Stalker / Ministra compatibility](stalker-ministra.md).
