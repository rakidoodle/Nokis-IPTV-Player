...
public partial class SeriesView : UserControl
{
    ...
    {
        if (DataContext is SeriesViewModel viewModel) viewModel.Player.FullScreenChanged += OnFullScreenChanged;
    }

    private void OnUnloaded(object sender, RoutedEventArgs e)
    {
        if (DataContext is SeriesViewModel viewModel)
        {
    ...
    {
        if (e.Key == Key.Escape && DataContext is SeriesViewModel { Player.IsFullScreen: true } viewModel)
        {
